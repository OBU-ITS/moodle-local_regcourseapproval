...
if ($mform_signup->is_cancelled()) {
	...
	if ($interim_user) {
		...
		if (time() < ($invitation->invitationdate + ($invitation->autoconfirm * 7 * 86400))) { // Within the 'auto-confirm' time limit?
			$authplugin->request_enrolment($USER, $approver_id, false, false); // sends enrolment request email (without displaying the notice)
			confirm_user($USER->username);
			enrol_user($USER->username, $course_id);
			$message = get_string('regapprovaluserconfirmation', 'local_regcourseapproval', array('firstname' => $USER->firstname));
		} else {
			$authplugin->request_enrolment($USER, $approver_id, false, true); // sends enrolment request email
			exit; // Shouldn't get here
		}
	} else { 
		$message = get_string('unrecogniseduser', 'local_regcourseapproval');
	}
} else if ($logged_in_user && $user_from_form = $mform_signup->get_data()) {
	...
	if ($valid_invitation) {
		...
		if (time() < ($invitation->invitationdate + ($invitation->autoconfirm * 7 * 86400))) { // Within the 'auto-confirm' time limit?
			$authplugin->request_enrolment($USER, $approver_id, true, false); // sends enrolment request email (without displaying the notice)
			confirm_user($USER->username);
			enrol_user($USER->username, $course_id);
			$message = get_string('regapprovaluserconfirmation', 'local_regcourseapproval', array('firstname' => $USER->firstname));
		} else {
